...
describe('#1609 — SWM snapshot-root memo (finalization reconcile)', () => {
  ...
  it('serves repeat absent-KA lookups from the index — no SWM rescan once ops are stamped', async () => {
    const store = new OxigraphStore();
    const chain = new MockChainAdapter();
    const fh = new FinalizationHandler(store, chain);
    ...
    const spy = vi.spyOn(fh as unknown as { getSharedMemoryQuadsForRoots: (...a: unknown[]) => unknown }, 'getSharedMemoryQuadsForRoots');

    // Cold pass: no stamps yet, so it recomputes (reads SWM quads for) both present ops.
    expect(await reconcileOne(chain, fh, 991n)).toBe('no-swm');
    expect(spy.mock.calls.length).toBeGreaterThanOrEqual(2);

    // Warm pass: a *different* absent KA. Fast path misses, and every present op is now
    // stamped, so the fallback scan excludes them all — zero SWM quad reads.
    spy.mockClear();
